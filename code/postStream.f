...
ccccccccccccc    m(zonal wave number) whereas the second dimension
ccccccccccccc    is n-m. In x and y the second dimension corresponds
...
ccccccccccccc    of zeros from equator to pole
     ...
     1  GS2RD,GS7E5,EPSLR,ESCONS,ESC1,ESC2,CI
      ...
      write(6,*)'enter grid point data set name'
      read(5,*)ifile2
      open(1,file=ifile2,form=
c    1'unformatted',access='direct',recl=ix*jx)
     1'unformatted',access='direct',recl=ix*jx)
      write(6,*)'enter spectral data set name'
      read(5,*)ifile
      open(66,file=ifile,form=
     1'unformatted')
      CALL BSCST
...
c
      ...
      phismn(mm,nj)=dnom*phismn(mm,nj)
      enddo
      enddo
c
c
